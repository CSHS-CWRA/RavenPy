...
class TestAssimilationGR4JCN:
    def test_simple(self, get_file):
        ...
        model, xa, hru_states, basin_states = assimilation_initialization(
            model_config,
            start_date=assim_start_date,
            end_date=assim_start_date + dt.timedelta(days=assim_step_days - 1),
            assim_var=assim_var,
            n_members=n_members,
        )
        ...
        q_assim, hru_states, basin_states = sequential_assimilation(
            ...
            q_obs,
            assim_var,
            start_date=assim_start_date + dt.timedelta(days=assim_step_days),
            end_date=assim_end_date,
            n_members=n_members,
