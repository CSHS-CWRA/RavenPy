...
class TestDistributedWorkflow:
    ...
    def test_simple_workflow(self, tmp_path, yangtze):
        ...
        df = self.routing_product.open_shapefile(shp_path)

        # Gauge station for observations at Matapedia
        # SubId: 175000128
        # -67.12542 48.10417
        sub = self.routing_product.upstream_from_coords(-67.12542, 48.10417, df)
        ...
        bm = self.routing_product.BasinMakerExtractor(
            df=sub,
        ...
        gw = self.routing_product.GridWeightExtractor(
            meteo_grid_fn,
            shp_path,
