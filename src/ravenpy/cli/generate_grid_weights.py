...
@click.option(
    ...
    nargs=2,
    type=click.Tuple([str, str]),
    default=GridWeightExtractor.VAR_NAMES,
    show_default=True,
...
@click.option(
    ...
    "--routing-id-field",
    type=str,
    default=GridWeightExtractor.ROUTING_ID_FIELD,
    show_default=True,
...
@click.option(
    ...
    "--netcdf-input-field",
    type=str,
    default=GridWeightExtractor.NETCDF_INPUT_FIELD,
    show_default=True,
    help="Attribute name in INPUT_FILE shapefile that defines the index of the shape "
    "in NetCDF model output file (numbering needs to be [0 ... N-1]).",
)
@click.option(
    "-g",
    "--gauge-id",
...
@click.option(
    ...
    type=float,
    default=GridWeightExtractor.AREA_ERROR_THRESHOLD,
    show_default=True,
    help="Threshold (as fraction) of allowed mismatch in areas between subbasins from routing information (ROUTING_FILE) "
    "and overlay with grid-cells or subbasins (INPUT_FILE). If error is smaller than this threshold the weights will be "
    "adjusted such that they sum up to exactly 1. Raven will exit gracefully in case weights do not sum up to at least 0.95.",
)
@click.option(
    "-o",
    "--output",
