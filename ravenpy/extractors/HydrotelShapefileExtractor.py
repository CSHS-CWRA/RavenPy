...
class HydrotelShapefileExtractor:
    ...
    def extract2(self,routingproductoutput) -> SoilProfilesCommand.Record:
        ...
        for _, row in self._df.iterrows():
            soil_recs.add(self._extract_soillayer_thickness(row)) #here we find the unique rows
        soils = list(soil_recs)
        soilinfo = []
        keyss = ['soilprofile']
        for soil in soils:
            soilinfo.append(SoilProfilesCommand.Record(soil[0],['TOPSOIL','FAST_RES','SLOW_RES'],soil[2:]))
        ss = dict.fromkeys(keyss,soilinfo)
        return (routingproductoutput.update(ss))

    def _extract_soillayer_thickness(self,row):

        items = ["TOPSOIL", "Nhorizons", "th1", "th2", "th3"]  # all 3 layers have the same soil type (Hydrotel configurations)
